...
class AsesoriaController extends Controller
{
    ...
    public function getEnrolledSubject(Request $request)
    {
        ...
        $objectEnrolled = $this->getObjectSubjectSchules(
            $idUser,
            $carreraId,
            $ciclo
        );
        ...
        return response()->json(
            [
                "message" =>
                    "Por el momento el alumno no posee una inscripcion activa",
            ],
            402
        );
    }
    ...
    public function saveRegistroSubject(Request $request)
    {
        ...
        if (!isset($params["codCargas"])) {
            return response()->json(
                [
                    "message" =>
                        "Debes seleccionar por lo menos una materias para poder inscribir",
                ],
                402
            );
        }
        ...
        if (isset($find)) {
            return response()->json(
                [
                    "message" =>
                        "El alumno ya pose una inscripcion para el ciclo actual",
                ],
                402
            );
        }
    ...
    private function getObjectSubjectSchules(
        $carnet,
        $carrera,
        $ciclo = "02-2021"
    ) {
    ...
    private function solicitudes(
        ...
        if (strcmp($equal, "=") === 0) {
            foreach ($dbResult as $value) {
                $value->carga = DB::table(
                    "solicitudes_cargas_academicas as sca"
                )
                    ->join(
                        "cargaacademica as c",
                        "c.codcarga",
    ...
    private function resolveSubjetcs($ganadas, $pendientes)
    {
        ...
        $posiblesMaterias = array_filter($pendientes, function ($item) use (
            $codigoSubject
        ) {
